...
export const useResize = (domRef: RefObject<HTMLDivElement | null>) => {
  useEffect(() => {
    ...
    // 컴포넌트 마운트 시 초기 높이 설정
    adjustGameViewHeight();

    // 윈도우 리사이즈 이벤트에 리스너 등록
    window.addEventListener("resize", adjustGameViewHeight);

    // 컴포넌트 언마운트 시 이벤트 리스너 해제 (클린업 함수)
    return () => {
      window.removeEventListener("resize", adjustGameViewHeight);
    };
  }, [domRef]);
};
